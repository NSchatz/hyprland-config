...
6. **Semantic lints (defect-class hardening)** and
7. **Cross-surface coherence checks.** Syntax validity is not correct output. These two steps
   are a catalog of defect classes found in production shakedowns - layer namespaces, expected
   binds, helper-script wiring, render-manifest completeness, the rofi 9-state element matrix.

   **Read** `${CLAUDE_PLUGIN_ROOT}/skills/rice/references/_shared/validator-lints.md` when you
   reach this step, and apply every lint in it. It is kept out of this prompt because the
   prompt loads in full on every invocation, including runs that stop before step 6.
