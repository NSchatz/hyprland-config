...
`require(...)` instead of `source =`. As of **0.55, Lua is the config language** and the wiki has
switched to it; hyprlang `.conf` is supported for **1 - 2 releases starting from 0.55** and is
dropped after that, so it is a closing window. This engine no longer assumes `.conf`: which
language it emits is resolved from the detected version by
`${CLAUDE_PLUGIN_ROOT}/skills/rice/scripts/config-language.sh`. The `.conf` form below remains
fully valid on 0.54.3 and, for now, 0.55.)
