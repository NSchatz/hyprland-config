...
|---|---|---|---|---|
| `hyprexpo` | Workspace overview (grid exposé). | `sandwichfarm/hyprexpo` (community fork — removed from official). | no | yes (commented) |
| `hy3` | i3/sway tree tiling. | `outfoxxed/hy3`. | **yes** (`layout = hy3`) | yes (commented) |
| `split-monitor-workspaces` | Per-monitor workspaces (each display gets its own 1–10). | `Duckonaut/split-monitor-workspaces`. | no | yes (commented; rebinds 1–10) |
| `hyprbars` | Per-window CSD title bars. | `hyprwm/hyprland-plugins`. | no | no (uses `hyprbars-button` config keys) |
| `pyprland` | Dropdown scratchpads + expose / magnify. | pip / AUR (NOT hyprpm). | no | yes (commented `pypr toggle …`) |
| `borders-plus-plus` | Extra window border rings. | `hyprwm/hyprland-plugins`. | no | no |
| `hyprtrails` | Motion trails. | community forks only (removed from official). | no | no |
| `hyprwinwrap` | Run an app as the wallpaper. | `hyprwm/hyprland-plugins`. | no | no |
...
| `hyprland-package-installer` | Reads `enabled`. When true, adds the build toolchain (`base-devel cmake meson cpio`) from `packages.md`. Reads `selected`: when `pyprland` is in it, adds the AUR `pyprland` package. **No package is added for any hyprpm plugin** — they are not packages. |
| rice skill (Mode A) | Reads `enabled` + `selected` to print the **user-run** `hyprpm` command block (one `hyprpm add` per repo, one `hyprpm enable` per plugin on its own line, framed by `mkdir -p ~/.local/share/hyprpm` and `hyprpm reload`). Claude does **not** execute these. |
