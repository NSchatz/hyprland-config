...
- **ml4w** (`mylinuxforwork/dotfiles`): theme folders under `~/.config/waybar/themes/<name>/` each with their own `config`/`style.css`/`modules.json`. Override by making `config-custom` / `style-custom.css`. Colors fed by matugen — ML4W's matugen template (`dotfiles/.config/matugen/templates/colors.css`) emits the full **Material You name set** (`@primary`, `@on_primary`, `@surface`, `@surface_container`, `@surface_container_high`, `@error`, `@inverse_primary`, `@tertiary`, …) plus two pre-alpha'd helpers `@blur_background` / `@blur_background8`. ML4W's default `style.css` was forked from the upstream Waybar sample so it still uses **`button.focused` (Sway)** instead of `button.active` — a real-world bug that ships in 2026 (see `gotchas.md`). It also lists `font-family: "Fira Sans Semibold", "Font Awesome 7 Free", …` with **no Nerd Font** — relies entirely on FontAwesome glyph fallback.
- **binnewbs** (`binnewbs/arch-hyprland`): JaKooLit-style `Modules`/`ModulesCustom`/`ModulesGroups` split with a matugen-rendered `colors.css` (Material You names — `@background`/`@primary`/`@on_surface_variant`/`@surface_container_high`/etc.) imported at the top of the user `style.css`. The matugen template is the 5-line jinja2 `<* for name, value in colors *> @define-color {{name}} {{value.default.hex}}; <* endfor *>` — the entire Material You palette wired in one loop.
- **dusky** (`dusklinux/dusky`): two themed bar variants in `.config/waybar/{01_mechabar_h,02_reminiscent_h}/` each with their own `config.jsonc` + `style.css`, importing a shared `matugen/generated/waybar-colors.css` (one dir up from the variant). The `02_reminiscent_h/style.css` carries a **transparency toggle marker** — paired `@define-color` blocks bracketed by `/* WAYBAR_OPAQUE_SWITCH_START */ … /* WAYBAR_OPAQUE_SWITCH_END */` comments; a `theme.sh` flips the second block in/out to toggle glassy↔opaque without rewriting selectors. The mechabar variant follows mechabar's outer-frame trick — `#waybar { background-color: @glass_outline } #waybar > box { margin: 4px; background-color: @glass_bar }` — using the 4px reveal as the visible ring (no `border` rule).
- **Catppuccin** (`catppuccin/waybar`): drop `mocha.css` next to `style.css`, `@import "mocha.css";` at top, reference `@text`/`@base`/`@mauve` etc., and use `alpha()`/`shade()` for translucency. The mocha.css emits 26 Catppuccin-canonical names (`@rosewater @flamingo @pink @mauve @red @maroon @peach @yellow @green @teal @sky @sapphire @blue @lavender @text @subtext1 @subtext0 @overlay2 @overlay1 @overlay0 @surface2 @surface1 @surface0 @base @mantle @crust`) — none of them match the rice engine's `@bg/@fg/@accent/...` names, so a verbatim Catppuccin port `style.css` will silently un-theme under the rice engine unless rewired (use the engine's `colors.css` and let the palette-from-scheme path pick the Catppuccin hexes).
...
- *Corner-weld via asymmetric radius pair* (linuxmobile/hyprland-dots): on a transparent-background bar with a square overall shape, give the leftmost module `border-radius: 0px 24px 0px 0px` (only top-right rounded) and the rightmost `border-radius: 0px 0px 0px 24px` (only bottom-left rounded) — the modules visually weld into the screen corners as if the bar weren't there. Works only with `margin: 0` and a transparent `window#waybar` bg.
- *Comment-marker transparency toggle* (dusky `02_reminiscent_h`): keep two `@define-color` blocks for the same names — one glassy (`alpha(@surface, 0.6)` …), one bracketed by `/* WAYBAR_OPAQUE_SWITCH_START */ … /* WAYBAR_OPAQUE_SWITCH_END */` with the solid form. A theme script un/comments the marked block to flip the bar opaque without rewriting selectors. Useful when the engine can't conditionalize; for our rice the engine just renders two different palettes instead.
- *Tiered island depth from a single palette* (dusky `01_mechabar_h`): give one row of modules `@surface_container_low`, the next `@surface`, the inner pair `@surface_container_high`, and the apex (the distro logo / clock) `@secondary` — driven purely by stacking Material-You container colors, no alpha or shadows needed. Reads as a layered ziggurat from one wallpaper-derived palette.
...
- *JetBrainsMono stylistic-set features* (every JaKooLit style.css): `font-feature-settings: '"zero", "ss01", "ss02", "ss03", "ss04", "ss05", "cv31"'` enables the slashed/dotted zero and the alt-`l`/`i`/`g`/`r`/`@`/`$` stylistic sets — substantially crisper at 13–14px on bars. No font swap needed; works on any JetBrainsMono variant.
- *`color0..color15` ANSI-16 palette as the lingua franca* (JaKooLit `colors-waybar.css`): the wallust template emits the full ANSI 16-color block (`@color0` … `@color15`) plus `@foreground`/`@background`/`@background-alt rgba(...,0.25)`/`@cursor`. Every JaKooLit style then references `@color12` (blue), `@color9` (red), etc. by index instead of named hues — the same template drives a Nord, Catppuccin, Rose-Pine, or wallpaper-derived skin without changing the CSS. The rice engine's `palette.conf` already reserves the same `color0..color15` slot — keep waybar styles palette-agnostic by referencing the index when the hue role matters more than the literal hue.
...
- *`"fixed-center": true` for true-centered modules-center* (every JaKooLit layout, HyDE): without it, `modules-center` slides off-axis whenever `modules-left` and `modules-right` have unequal widths (the case as soon as the clock and a wide tray coexist). Setting it pins the center group to the bar's geometric center; cheap fix every popular config ships.
- *`"include": [...]` modular module split* (HyDE, JaKooLit, binnewbs, Matt-FTW): every popular waybar rice ships modules in separate files — JaKooLit's six-file split (`Modules` `ModulesWorkspaces` `ModulesCustom` `ModulesGroups` `UserModules` `ModulesVertical`), Matt-FTW's per-module `modules/*.jsonc` (each module in its own file), HyDE's `~/.config/waybar/modules/`. Same module config gets reused across N bar layouts via one `include`. Our rice's writer-owned single-file output is intentionally simpler (one generator → one file), but `include` is the de-facto community standard and is required if the user wants to keep their own `UserModules`-style override — the writer should NOT clobber a `~/.config/waybar/UserModules` file if one exists. See `gotchas.md`.
- *Swappable bar-form via one `include`* (Matt-FTW): root `config.jsonc` is just `{ "include": "~/.config/waybar/bars/vertical-bar.jsonc", "layer": "top", "reload_style_on_change": true }`; per-form specs live in `bars/{top,bottom,vertical}-bar.jsonc`. Each per-form file re-includes the same `modules/*.jsonc` set, so module configs stay defined once. Cleanest "swap top↔vertical without duplicating module configs" recipe in the corpus.
...
- **Heavy `box-shadow` on small rounded translucent pills renders as a visible rectangular halo** on common GTK/cairo stacks (the cairo blur falls back to a non-blurred bounding-box stamp when the radius exceeds a per-stack threshold; readers see a hard-edged dark rectangle, not a soft drop shadow). Defaults in this plugin ship `box-shadow: none` for the floating-islands and separated-pills archetypes. The "elevation" idioms in *Battle-tested techniques* above are opt-in — use a single light layer (`0 1px 2px rgba(0,0,0,0.20)`) on flat pills, save the heavy multi-layer stacks for the `glassmorphism` / `frosted` archetypes that are already paired with `layerrule blur`.
- **Module divider on the first child reads as a stray vertical line** floating against the bar background. The divider recipe (`border-left: 1px solid alpha(@accent,…)` between stat modules, mechabar-style) must use `:not(:first-child)` so it applies only between modules, never at the leading edge. Frequently-empty modules — `mpris` (silent when nothing's playing), `tray` (empty until an SNI app registers), `idle_inhibitor` (empty until exec'd), any `custom/*` whose `format` evaluates to blank — change *which* module is first when they collapse, so an explicit `#pulseaudio { border-left: … }` list is fragile. Prefer the structural `:not(:first-child)` form. See `template.md` → *Module dividers* for the canonical block.

## Provenance

Citations for this file live at `.research/sources/components-waybar-styling.md` (repo root), kept out of
