...
shell is pointed at it. The user's own `~/.config/starship.toml` is left untouched. (Corpus
evidence: `dusklinux/dusky` ships a matugen template `starship-colors.toml` that only emits a
`[palettes.colors]` block, but the entry in `.config/matugen/config.toml` that would render it
is **commented out** — there's no clean way to splice a starship palette block into an existing
starship.toml from an external file. Their workaround was an offline post-hook `ln -nfs` of the
generated palette over the user's `starship.toml`, which we improve on by writing the whole
config to a rice path and pointing `STARSHIP_CONFIG` at it.)
...
A representative cross-section of the top-19 Hyprland dotfiles (per `/workspace/.research/corpus.md`)
informed the three `.tmpl` files in this folder. Concrete, attributed picks:

- **`command_timeout`** (`starship.tmpl`) — `basecamp/omarchy` `config/starship.toml` ships
  `command_timeout = 200` so a slow VCS/network module never hangs the prompt. We set 500 to
  give git status a wider safe window; users who add cloud (`gcloud`/`aws`/`azure`) modules
  should keep it ≤ 1000 or disable those modules.
- **Ahead/behind/diverged glyph dict** (`starship.tmpl` `[git_status]`,
  `oh-my-posh.tmpl` git segment template) — `⇡${count}` / `⇣${count}` /
  `⇕⇡${ahead_count}⇣${behind_count}` appears in `omarchy/config/starship.toml`,
  `prasanthrangan/hyprdots Configs/.config/starship/starship.toml` (HyDE's powerline preset),
  and `mylinuxforwork/dotfiles dotfiles/.config/ohmyposh/zen.toml` (omp template mirrors the
  same ⇡⇣ pair). Recolor by ahead/behind via `foreground_templates` on omp.
- **`transient_prompt`** (`oh-my-posh.tmpl`) — `mylinuxforwork/dotfiles
  .config/ohmyposh/zen.toml` ships a `[transient_prompt]` block that collapses past prompts to
  a bare `❯` with a green-on-success / red-on-failure recolor. The starship analogue is
  `caelestia-dots/caelestia fish/config.fish`'s `starship_transient_prompt_func` +
  `enable_transience` — fish-only sugar, so we leave it to the user (not in the rendered
  `.tmpl`) and reference it in `styling.md` instead.
- **`type -q <tool>; and …`** (`config.fish` block) — `Matt-FTW/dotfiles
  .config/fish/conf.d/starship.fish` (`if type -q starship; starship init fish | source; end`)
  and `caelestia-dots/caelestia fish/config.fish` both guard every external init with
  `type -q`. Our template uses the inline `; and` form to keep the managed block compact.
- **Fragmented vs unified init** — `Matt-FTW/dotfiles` splits each tool's init into its own
  `~/.config/fish/conf.d/<tool>.fish` (`starship.fish`, `zoxide.fish`, `atuin.fish`); `caelestia`
  and `end-4/dots-hyprland` keep everything in one `config.fish`. Our managed-block design picks
  the unified shape because a single block is easier to find-and-replace on `rice apply` and
  doesn't fight with whatever the user already has under `conf.d/`.
- **Stale `set -U` is real** — `end-4/dots-hyprland dots/.config/fish/fish_variables` ships a
  full set of `SETUVAR fish_color_*` lines (`SETUVAR fish_color_command:blue`, etc.). If a user
  layered that rice and then ran ours, a bare `set fish_color_command …` in our snippet would
  silently update those universals; explicit `set -g` on every line in `fish.tmpl` defeats it.
- **atuin Up-arrow rebind** — `Matt-FTW/dotfiles .config/fish/conf.d/atuin.fish` runs
  `atuin init fish | source` then `bind up _atuin_bind_up` (and the `\e[A` / `\eOA` variants
  for terminals that send a different escape). This is a deliberate atuin idiom; the user can
  add it post-install if they want history-aware Up. Our template only emits the init line —
  rebinding is noted in `styling.md` so the user opts in.
