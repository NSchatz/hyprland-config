...
| `terminal.cursor.shape` | string (enum) | `block` \| `beam` \| `underline`. Maps to per-emulator key (`cursor_shape`, `[cursor] style`, `[cursor] style`, `default_cursor_style`, `cursor-style`). |
| `terminal.cursor.blink` | bool | `true` adds a blink interval (kitty `cursor_blink_interval`, ghostty `cursor-style-blink`, etc.); `false` leaves the cursor steady. |
