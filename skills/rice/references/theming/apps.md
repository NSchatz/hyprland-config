...
| firefox | `components/browser/` (opt-in) | `<firefox-profile>/chrome/rice-colors.css` (loaded by static `userChrome.css` via `@import`; resolved by `firefox-bootstrap.sh`) | `:root{--rice-*}` block: `{{bg}} {{fg}} {{surface}} {{muted}} {{accent}} {{accent2}}` + the 6 ANSI variants per `firefox.tmpl` | `bash ~/.config/hypr-rice/firefox-restart.sh` (pkill + relaunch detached; session restore brings tabs back) or empty for "(next launch)" |
...
| eww | `components/widgets/` | `~/.config/eww/colors.scss` (`@import "colors";`) | `$bg $fg $surface $muted $cursor $accent $accent2 $red $green $yellow $blue $magenta $cyan $color0..$color15` | `pgrep -x eww >/dev/null && eww reload \|\| true` (guarded so the line no-ops when the daemon isn't running) |
