...
while IFS=$'\t' read -r name tmpl out rcmd next_hint; do
    ...
    # A manifest row's `~/.config/<app>/...` names a CONFIG SURFACE, so it resolves under the
    # same base every other write here uses; every other `~/...` path still expands to $HOME.
    tmpl="$(xdg_expand_path "$tmpl")"; out="$(xdg_expand_path "$out")"
...
write_restore_script() {
    local engine="${RICE_THEMING_ENGINE:-}" out reapply hypr_dir
    # Same base as every other config write here, and the same HYPR_DIR override the rest
    # of the plugin honours: this file goes INSIDE the Hyprland config dir.
    hypr_dir="$(xdg_config_path hypr "${HYPR_DIR:-}")" || {
        echo "RESTORE_SCRIPT_SKIPPED no config directory could be determined" >&2
        return 0
    }
    out="$hypr_dir/scripts/restore-theme.sh"
    case "$engine" in
    ...
    mkdir -p "$(dirname "$out")"
    # Backslash-escape every $var we want literal in the output; ${reapply} and ${RICE_DIR}
    # expand HERE. The palette path is baked in as the absolute path this pass resolved:
    # a login script must not have to re-derive it from an environment that may not export
    # XDG_CONFIG_HOME yet.
    cat > "$out" <<EOF
#!/usr/bin/env bash
