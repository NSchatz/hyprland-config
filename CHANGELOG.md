...
Preview a generated desktop in a throwaway VM before it touches your machine.

`rice preview start` boots the staged rice inside QEMU, on its own virtual GPU, with
its own seat and its own keyboard. You connect to it when you want to, look at it and
drive it, and your own session is never involved: nothing is written to your home
directory and your compositor is never reconfigured. The VM is a qcow2 overlay on a
cached base, so discarding a preview is deleting a file.

### Why a VM and not a container

Three architectures were built and measured before this one survived, and the reasons
are recorded because each rejection is a fact about Hyprland rather than a preference.

- **A container cannot run Hyprland headless.** Aquamarine builds its GBM allocator
  only from a backend that exposes a DRM fd (`Backend.cpp:163-179`), and
  `CHeadlessBackend::drmFD()` returns `-1` (`Headless.cpp:133`). The allocator comes
  from the DRM backend, meaning a `card` node, or from a parent compositor, and from
  nothing else. Pinning a render node is refused with
  `drm: Explicit device /dev/dri/renderD128 not found`.
- **Nesting in the user's own session works, and was rejected anyway.** It rendered
  pixel-exact, but it shares the seat: it takes focus, it cannot test a `SUPER` bind
  without editing the host config, and a nested compositor whose window is hidden
  receives no frame callbacks, renders nothing, and hangs any screenshot of it.
- **An isolated container needs a parent compositor Aquamarine will accept.** It binds
  Wayland globals at hardcoded, unnegotiated versions (`Wayland.cpp:121-136`):
  `wl_compositor` 6, `xdg_wm_base` 6, `wl_seat` 9, `dmabuf` 4. sway 1.12 offers
  `xdg_wm_base` 5 and weston 15 offers `wl_compositor` 5, so every wlroots parent is
  rejected outright. mutter 50.5 satisfies all four, and then every swapchain buffer
  allocation failed on NVIDIA with `GBM: Failed to allocate a GBM buffer: bo null`.

A VM sidesteps all of it: the guest has a virtual KMS device of its own, so Hyprland
runs its own DRM backend with no parent compositor and no protocol coupling.

### The preview

- **`scripts/rice-preview.sh`** (new) plus `scripts/ricelib/preview/`: `vm` (the QEMU
  argv and lifecycle), `guestimage` (the three-layer disk), `guest` (the ssh control
  channel), `rfb` (screen capture), `session` (the lock), `previewcli`.
- **The guest is given nothing of the host's.** No KMS device, no PCI or USB
  passthrough, VNC and the ssh forward bound to loopback only, and the staged config
  shared read-only so a preview cannot write back into the tree being edited. This is
  enforced in `vm.qemu_argv`, which raises rather than warns.
- **Screenshots are taken from outside, over VNC**, not with `grim` inside the guest.
  A capture that waits on the guest to render a frame hangs exactly when a preview is
  least healthy, which is when a screenshot is most wanted.
- **A config that autostarts nothing is reported as such.** An empty desktop and a
  broken VM look identical, so `start` counts the staged `exec-once` entries and says
  when there are none, and `rice preview demo` launches the surfaces the config
  actually themes, printing every one it started.

### Tests

`tests/test_preview_vm_argv.sh`, `tests/test_preview_session.sh` and
`tests/test_preview_autostart.sh` (40 assertions). The isolation and lock assertions
were negative-controlled: the guarantees were deliberately broken and the suite went
red on exactly the assertions that name them.

### Not yet

Guest provisioning is not codified. `guestimage` fetches the cloud image, builds the
overlay and writes the cloud-init seed, but the step that installs Hyprland into the
guest and writes its boot profile has only been performed by hand, so a fresh machine
cannot yet reproduce the cached image. `rice preview apply`, `--git-ref`, the skill
document and the offer inside the rice flow are also still to come.
