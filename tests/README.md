...
| `test_preview_vm_argv.sh` | `scripts/ricelib/preview/vm.py` - the isolation properties of the preview VM, read off the generated QEMU argv: **no host KMS device, no PCI or USB passthrough**, VNC and the ssh forward bound to loopback only, the overlay booted rather than the cached base, the staged config shared read-only, and the guards raising rather than warning |
| `test_preview_session.sh` | `scripts/ricelib/preview/session.py` - the one-at-a-time lock, a stale pidfile reading as stopped, **a record that outlives its VM** (what a crash leaves behind), idempotent stop, the throwaway overlay being discarded, and a corrupt record reading as no record |
| `test_preview_autostart.sh` | `rice preview` reporting a config that autostarts nothing, so an empty desktop is never mistaken for a broken VM - including that a `bind = ..., exec, ...` is **not** counted as an autostart |
...
| `test_xdg_resolver.sh` | `scripts/ricelib/xdg.py`, the ONE decision about where configuration lives, across 13 environments (absolute / relative / empty `XDG_CONFIG_HOME`, trailing and multiple slashes, HOME as root, HOME unset, both unset): the base resolves per the rule, an unresolvable environment REFUSES rather than inventing a path, an override wins outright, `~` expansion is config-aware (`~/.config/...` follows the base, `~/.cache` and `~/Pictures` do not), and a rejected relative value is announced rather than taken in silence. |
| `test_restorepoint_behaviour.sh` | Restore-point behaviours no other file pins: a symlink is backed up AS a link, a trailing slash is the same surface, the excluded dir follows `HYPR_DIR` in both directions, a path containing it is refused writing nothing, and re-enrolling within one apply keeps the FIRST backup rather than capturing the apply's own output. |
| `test_python_bootstrap.sh` | `scripts/ensure-python.sh` - present / absent / declined / no-pacman / failed each its own outcome, the accepted install is recorded, and the bootstrap never invokes the interpreter it installs. |
