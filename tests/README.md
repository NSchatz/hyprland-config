...
| `test_visual_themes.sh` | **Skipped by default** (set `RUN_VISUAL=1`). Builds an Arch + sway + waybar + wofi + mako + kitty + grim container, scaffolds the rice engine, and for each shipped preset (catppuccin-mocha, gruvbox, nord, tokyo-night) re-renders the palette + grim-screenshots waybar, wofi, mako notification, kitty terminal, and the full desktop composite. The screenshots land in `tests/visual/output/<preset>/` and CI uploads the dir as a `theme-screenshots` artifact — open any GitHub Actions run for the branch and download the zip to compare presets side-by-side. Sway is wlroots-based so it boots headlessly without `/dev/dri`, which is why this works on stock CI runners. |
