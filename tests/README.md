...
| `test_visual_themes.sh` | **Skipped by default** (set `RUN_VISUAL=1`). Consumes the committed `tests/agent-eval/generated/<fixture>/` dirs (real component-writer output, populated by the user running `/regression-eval` locally) — installs them into a sway+waybar+wofi+mako+kitty container, re-renders the palette per preset, screenshots the resulting desktop, and uploads `tests/visual/output/<preset>/*.png` as a `theme-screenshots` artifact. If no fixtures are committed yet, the test skips cleanly. See `tests/agent-eval/README.md` for the local→CI loop. |
