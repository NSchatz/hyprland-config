...
| `test_xdg_parity.sh` | The bash and Python config-path resolvers give the SAME answer across 13 environments (absolute / relative / empty `XDG_CONFIG_HOME`, trailing and multiple slashes, HOME as root, HOME unset, both unset). `xdg-config.sh` stays bash because `detect-version.sh` and the whole write path source it before python exists, so the two implementations are allowed only with this proof. |
