...
| `test_integration_docker.sh` | **Skipped by default.** Builds Arch + Hyprland container, starts Hyprland under `WLR_BACKENDS=headless`, runs `safe-apply.sh` against a known-good staging config, asserts `SAFE_APPLY=ok` + `hyprctl configerrors` empty + no `[ERR]`/`[CRITICAL]` lines in the Hyprland log |
